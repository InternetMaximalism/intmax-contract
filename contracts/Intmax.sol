...
contract Intmax is Storage, ReentrancyGuard, Events, Config {
    ...
    function depositERC20(IERC20 _token, uint256 _amount, address _intmaxAddress) external nonReentrant notFreeze {
    ...
    function registerDeposit(uint16 _tokenId, uint256 _amount, address _intmaxAddress) internal {
    ...
    ///         NOTE: We will call ERC20.transfer(.., _amount), but if according to internal logic of ERC20 token Intmax contract
    ///         balance will be decreased by value more then _amount we will try to subtract this value from user pending balance
    function withdrawPendingBalance(address payable _owner, address _token, uint256 _amount) external nonReentrant {
    ...
    function registerWithdrawal(uint16 _token, uint256 _amount, address payable _to) internal {

    }

    function exit(address payable _owner, address _token, uint256 _amount) external {
